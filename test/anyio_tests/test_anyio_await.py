...
from motor.motor_anyio import (AnyIOMotorClientSession,
...
class TestAnyIOAwait(AnyIOTestCase):
    @anyio_test
    ...
    async def test_iter_gridfs(self):
        gfs = AnyIOMotorGridFSBucket(self.db)
    ...
    async def test_stream_to_handler(self):
        # Sort of Tornado-specific, but it does work with anyio.
        fs = AnyIOMotorGridFSBucket(self.db)
        content_length = 1000
        await fs.delete(1)
    ...
    async def test_session(self):
        ...
        self.assertIsInstance(s, AnyIOMotorClientSession)
