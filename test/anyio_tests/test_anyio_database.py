...
from motor.motor_anyio import (AnyIOMotorCollection, AnyIOMotorDatabase)
from test import env
from test.anyio_tests import (AnyIOTestCase, anyio_test)


class TestAnyIODatabase(AnyIOTestCase):
    @anyio_test
    async def test_database(self):
        # Test that we can create a db directly, not just get on from
        # AnyIOMotorClient.
        db = AnyIOMotorDatabase(self.cx, 'motor_test')
    ...
    def test_collection_named_delegate(self):
        ...
        self.assertTrue(isinstance(db['delegate'], AnyIOMotorCollection))
        db.client.close()
    ...
    async def test_create_collection(self):
        ...
        # AnyIOMotorCollection, creating it again raises CollectionInvalid.
        db = self.db
        await db.drop_collection('test_collection2')
        collection = await db.create_collection('test_collection2')
        self.assertTrue(isinstance(collection, AnyIOMotorCollection))
        self.assertTrue(
    ...
    async def test_drop_collection(self):
        # Make sure we can pass an AnyIOMotorCollection instance to
        # drop_collection.
        db = self.db
    ...
    def test_get_collection(self):
        ...
        self.assertTrue(isinstance(coll, AnyIOMotorCollection))
    ...
    def test_with_options(self):
        ...
        self.assertTrue(isinstance(db2, AnyIOMotorDatabase))
