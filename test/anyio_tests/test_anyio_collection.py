...
from motor.motor_anyio import (AnyIOMotorCollection,
...
class TestAnyIOCollection(AnyIOTestCase):
    @anyio_test
    async def test_collection(self):
        # Test that we can create a collection directly, not just from
        # database accessors.
        collection = AnyIOMotorCollection(self.db, 'test_collection')
    ...
    async def test_insert_many_one_bad(self):
        ...
        self.assertEqual(
            set([1, 2]),
            set((await collection.distinct('_id'))))
    ...
    async def test_unacknowledged_insert(self):
        ...
        await coll.with_options(write_concern=WriteConcern(0)).insert_one(
            {'_id': 1})
    ...
    async def test_map_reduce(self):
        ...
        # First do a standard mapreduce, should return AnyIOMotorCollection
        collection = self.collection
        tmp_mr = await collection.map_reduce(map_fn, reduce_fn, 'tmp_mr')

        self.assertTrue(
            isinstance(tmp_mr, motor_anyio.AnyIOMotorCollection),
            'map_reduce should return AnyIOMotorCollection, not %s' % tmp_mr)
    ...
    def test_with_options(self):
        ...
        self.assertTrue(isinstance(coll2, AnyIOMotorCollection))
