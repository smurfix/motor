...
from motor.frameworks.anyio import Future, as_future, call_soon
...
class TestAnyIOCursor(AnyIOMockServerTestCase):
    def test_cursor(self):
        cursor = self.collection.find()
        self.assertTrue(isinstance(cursor, motor_anyio.AnyIOMotorCursor))
    ...
    async def test_fetch_next_exception(self):
        ...
        with warnings.catch_warnings(record=True), self.assertRaises(OperationFailure):
            await cursor.fetch_next

        # Avoid the cursor trying to close itself when it goes out of scope.
        cursor.delegate._Cursor__id = None

    @anyio_test(timeout=30, session=True)
    ...
    async def _test_cancelled_error(self, coro):
        ...
        async with FailPoint(self.cx, fp):
            ...
            task.cancel()
            with self.assertRaises(anyio.CancelledError):
                await task
            await cleanup()
    ...
    async def test_exhaust(self):
        ...
        client = self.anyio_client(maxPoolSize=1)
        # Ensure a pool.
    ...
    async def test_aggregate_batch_size(self):
        ...
        cx = self.anyio_client(event_listeners=[listener])
...
class TestAnyIOCursorMaxTimeMS(AnyIOTestCase):
    def setUp(self):
        super().setUp()
        anyio.run(self.maybe_skip)

    def tearDown(self):
        anyio.run(self.disable_timeout)
        super().tearDown()
