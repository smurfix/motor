...
class TestAnyIOClient(AnyIOTestCase):
    ...
    def test_database_named_delegate(self):
        ...
        self.assertTrue(isinstance(self.cx['delegate'],
                                   motor_anyio.AnyIOMotorDatabase))

    @anyio_test
    async def test_reconnect_in_case_connection_closed_by_mongo(self):
        cx = self.anyio_client(maxPoolSize=1, retryReads=False)
    ...
    async def test_connection_failure(self):
        ...
        client = motor_anyio.AnyIOMotorClient('localhost', 8765,
    ...
    async def test_max_pool_size_validation(self):
        with self.assertRaises(ValueError):
            motor_anyio.AnyIOMotorClient(maxPoolSize=-1,
                                             io_loop=self.loop)

        with self.assertRaises(ValueError):
            motor_anyio.AnyIOMotorClient(maxPoolSize='foo',
                                             io_loop=self.loop)

        cx = self.anyio_client(maxPoolSize=100)
        self.assertEqual(cx.max_pool_size, 100)
        cx.close()

    @anyio_test(timeout=30)
    async def test_drop_database(self):
        # Make sure we can pass an AnyIOMotorDatabase instance
        # to drop_database
    ...
    def test_get_database(self):
        ...
        assert isinstance(db, motor_anyio.AnyIOMotorDatabase)
    ...
    async def test_list_databases(self):
        ...
        self.assertIsInstance(cursor, motor_anyio.AnyIOMotorCommandCursor)
    ...
    async def test_contextvars_support(self):
        ...
        listener = Listener()
        client = self.anyio_client(event_listeners=[listener])
...
class TestAnyIOClientTimeout(AnyIOMockServerTestCase):
    @anyio_test
    async def test_timeout(self):
        server = self.server(auto_ismaster=True)
        client = motor_anyio.AnyIOMotorClient(server.uri,
                                                  socketTimeoutMS=100,
                                                  io_loop=self.loop)
...
class TestAnyIOClientHandshake(AnyIOMockServerTestCase):
    @anyio_test
