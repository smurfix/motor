...
class AIOMotorTestBasic(AnyIOTestCase):
    def test_repr(self):
        self.assertTrue(repr(self.cx).startswith('AnyIOMotorClient'))
        self.assertTrue(repr(self.db).startswith('AnyIOMotorDatabase'))
        self.assertTrue(repr(self.collection).startswith(
            'AnyIOMotorCollection'))

        cursor = self.collection.find()
        self.assertTrue(repr(cursor).startswith('AnyIOMotorCursor'))
    ...
    async def test_write_concern(self):
        ...
        for gle_options in [
            ...
            cx = self.anyio_client(test.env.uri, **gle_options)
    ...
    async def test_read_preference(self):
        # Check the default
        cx = motor_anyio.AnyIOMotorClient(test.env.uri, io_loop=self.loop)
    ...
    def test_underscore(self):
        ...
        self.assertIsInstance(self.db['_collection'],
                              motor_anyio.AnyIOMotorCollection)
    ...
    async def test_inheritance(self):
        class CollectionSubclass(motor_anyio.AnyIOMotorCollection):
            pass

        class DatabaseSubclass(motor_anyio.AnyIOMotorDatabase):
            def __getitem__(self, name):
                return CollectionSubclass(self, name)

        class ClientSubclass(motor_anyio.AnyIOMotorClient):
            def __getitem__(self, name):
