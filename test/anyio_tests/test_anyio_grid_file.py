...
from test.anyio_tests import anyio_test, AnyIOTestCase
from motor.frameworks.anyio import Session


class MotorGridFileTest(AnyIOTestCase):
    ...
    @anyio_test(session=True)
    async def test_attributes(self):
        f = motor_anyio.AnyIOMotorGridIn(
            self.db.fs,
            filename="test",
            foo="bar",
            content_type="text")
        ...
        g = motor_anyio.AnyIOMotorGridOut(self.db.fs, f._id)
        attr_names = (
            '_id',
            'filename',
            'name',
    ...
    async def test_iteration(self):
        fs = motor_anyio.AnyIOMotorGridFSBucket(self.db)
        _id = await fs.upload_from_stream('filename', b'foo')
        g = motor_anyio.AnyIOMotorGridOut(self.db.fs, _id)
    ...
    async def test_basic(self):
        f = motor_anyio.AnyIOMotorGridIn(self.db.fs, filename="test")
    ...
    async def test_readchunk(self):
        in_data = b'a' * 10
        f = motor_anyio.AnyIOMotorGridIn(self.db.fs, chunkSize=3)
    ...
    async def test_gridout_open_exc_info(self):
        ...
        g = motor_anyio.AnyIOMotorGridOut(self.db.fs, "_id that doesn't exist")
        try:
            await g.open()
        except NoFile:
            _, _, tb = sys.exc_info()
    ...
    async def test_alternate_collection(self):
        ...
        f = motor_anyio.AnyIOMotorGridIn(self.db.alt)
        await f.write(b"hello world")
        await f.close()
        ...
        g = motor_anyio.AnyIOMotorGridOut(self.db.alt, f._id)
        self.assertEqual(b"hello world", (await g.read()))
    ...
    async def test_grid_in_default_opts(self):
        self.assertRaises(TypeError, motor_anyio.AnyIOMotorGridIn, "foo")

        a = motor_anyio.AnyIOMotorGridIn(self.db.fs)
    ...
    async def test_grid_in_custom_opts(self):
        self.assertRaises(TypeError, motor_anyio.AnyIOMotorGridIn, "foo")
        a = motor_anyio.AnyIOMotorGridIn(
        ...
        b = motor_anyio.AnyIOMotorGridIn(
            self.db.fs,
    ...
    async def test_grid_out_default_opts(self):
        self.assertRaises(TypeError, motor_anyio.AnyIOMotorGridOut, "foo")
        gout = motor_anyio.AnyIOMotorGridOut(self.db.fs, 5)
        ...
        b = await motor_anyio.AnyIOMotorGridOut(self.db.fs, a._id).open()
    ...
    async def test_grid_out_custom_opts(self):
        one = motor_anyio.AnyIOMotorGridIn(
            self.db.fs, _id=5, filename="my_file",
            contentType="text/html", chunkSize=1000, aliases=["foo"],
            metadata={"foo": 1, "bar": 2}, bar=3, baz="hello")
        ...
        two = await motor_anyio.AnyIOMotorGridOut(self.db.fs, 5).open()
    ...
    async def test_grid_out_file_document(self):
        one = motor_anyio.AnyIOMotorGridIn(self.db.fs)
        await one.write(b"foo bar")
        await one.close()

        file_document = await self.db.fs.files.find_one()
        two = motor_anyio.AnyIOMotorGridOut(
            self.db.fs, file_document=file_document)

        self.assertEqual(b"foo bar", (await two.read()))

        file_document = await self.db.fs.files.find_one()
        three = motor_anyio.AnyIOMotorGridOut(self.db.fs, 5, file_document)
        self.assertEqual(b"foo bar", (await three.read()))

        gridout = motor_anyio.AnyIOMotorGridOut(self.db.fs, file_document={})
        with self.assertRaises(NoFile):
            await gridout.open()
    ...
    async def test_write_file_like(self):
        ...
        two = motor_anyio.AnyIOMotorGridOut(self.db.fs, one._id)
        three = motor_anyio.AnyIOMotorGridIn(self.db.fs)
        await three.write(two)
        await three.close()

        four = motor_anyio.AnyIOMotorGridOut(self.db.fs, three._id)
        self.assertEqual(b"hello world", (await four.read()))

    @anyio_test(session=True)
    async def test_set_after_close(self):
        f = motor_anyio.AnyIOMotorGridIn(self.db.fs, _id="foo", bar="baz")
        ...
        g = await motor_anyio.AnyIOMotorGridOut(self.db.fs, f._id).open()
